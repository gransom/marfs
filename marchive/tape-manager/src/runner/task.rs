...
impl Drop for InnerTask {
    ...
    fn drop(&mut self) {
        ...
        if self.parsedoffset > 0 {
            ...
            self.objtable.borrow_mut().remove(&self.taskdef, self.timestamp);
            // possibly cleanup filtered taskfile ( if not already renamed )
            let filterpath = PathBuf::from(&ProcessingPath::new(Some(process::id()), Some(self.timestamp), ProcessingPathElement::FilteredTaskfile));
        ...
        match &self.cleanup {
            ...
            InnerTaskCleanup::Delete => {
                if let Err(error) = fs::remove_file(currentpath) {
                    eprintln!(
                        "ERROR: Failed to remove {currentpath:?}: {error}"
                    )
                }
            }
        };
        ...
        let mut ppath = ProcessingPath::new(Some(process::id()), Some(self.timestamp), ProcessingPathElement::IntermediateDir);
...
impl Task<TaskGrabbed> {
    ...
    pub fn new(currentpath: PathBuf, objtable: Rc<RefCell<ObjTable>>) -> Option<Self> {
        ...
        if !value_within_restriction(
                taskfile,
                "pod",
                &config.pods,
                &pathcaptures.name("pod")
            )  ||
        ...
        for value in &config.task_path_values {
            ...
            if let Some(_) = itask.pathvals.insert(value.clone(), String::from(capture.as_str())) {
                eprintln!(
        ...
        while itask.timestamp.duration_since(origts).unwrap() < Duration::from_millis(100) {
            ...
            let ppath = ProcessingPath::new(Some(process::id()), Some(itask.timestamp), ProcessingPathElement::IntermediateDir);
            ...
            if let Some(tsparent) = tsdirpath.parent() {
                if let Err(error) = fs::create_dir_all(tsparent) {
                    if ! matches!(error.kind(), ErrorKind::AlreadyExists) {
                        eprintln!("ERROR: Failed to create processing parent path {tsparent:?}: {error}");
                        return None;
                    }
                }
            }
            match fs::create_dir(&tsdirpath) {
                ...
                Err(e) => match e.kind() {
                    ...
                    _ => { eprintln!("ERROR: Failed to create processing timestamp path {tsdirpath:?}: {e}"); return None; }
                }
            }
        }
        let ppath = ProcessingPath::new(Some(process::id()), Some(itask.timestamp), ProcessingPathElement::OriginalTaskfile(taskfile.as_ref()));
        ...
        if let Some(tgtdir) = tgtpath.parent() {
            if let Err(error) = fs::create_dir_all(tgtdir) {
                match error.kind() {
                    ErrorKind::AlreadyExists => (), // ignore EEXIST
                    _ => {
                        eprintln!("ERROR: Failed to create processing dir location {tgtdir:?}: {error}");
                        return None;
                    }
                }
            }
        ...
        if let Err(error) = fs::rename(&itask.currentpath, &tgtpath) {
            eprintln!("ERROR: Failed to rename taskfile {:?} to processing location \
                      {tgtpath:?}: {error}", &itask.currentpath);
            return None;
        }
        println!("New taskfile {:?} has been renamed to processing location {tgtpath:?}", &itask.currentpath);
    ...
    pub fn filter(mut self) -> TaskResult<Task<TaskGrabbed>, Task<TaskFiltered>> {
        ...
        for key in &self.itask.taskdef.command_values {
            let Some(pval) = self.itask.pathvals.get(key) else { continue; };
            if let Some(oldval) = cmdvals.insert(String::from(key),String::from(pval)) {
                eprintln!("WARNING: Pre-existing {key}->{oldval} mapping \
        ...
        let ppath = ProcessingPath::new(Some(process::id()), Some(self.itask.timestamp), ProcessingPathElement::FilteredTaskfile);
        let filterpath = PathBuf::from(&ppath);
        let filterfile = match fs::OpenOptions::new()
                                              .append(true)
                                              .create(true)
                                              .open(&filterpath) {
            Err(error) => return TaskResult::Err(format!("failed to open filtered taskfile output {filterpath:?}: {error}")),
            Ok(file) => file,
        };
        ...
        for (index, sourceline) in sourcereader.lines().enumerate() {
            ...
            if index < self.itask.parsedoffset { continue; }
            ...
            for key in &config.task_content_values {
                ...
                if self.itask.taskdef.file_format_values.iter().any(|k| k == key) {
            ...
            let insertres = self.itask.objtable.borrow_mut().insert(object, &self.itask.taskdef, self.itask.timestamp);
            ...
            if let Err(error) = writeln!(filterwriter, "{sourceline}") {
                return TaskResult::Err(
                    format!("ERROR: Failed to write to filtered taskfile {filterpath:?}: {error}")
                );
            }
            // format our output and command strings
            let outputline = match format_line(&self.itask.taskdef.file_format, 
                                               &filevals) {
                Err(error) => return TaskResult::Err(
                    format!(
            ...
            let (subtask,writer) = match self.itask.subtasks
                                   .iter()
            ...
            if let Err(error) = writeln!(writer,"{outputline}") { // output our reformatted line
                return TaskResult::Err(
                    format!(
...
impl Task<TaskFiltered> {
    ...
    pub fn launch(mut self, proc_limit: &mut u32)
        ...
        for subtask in self.itask.subtasks.iter_mut() {
            if *proc_limit == 0 { return TaskResult::Unchanged(self); }
            match subtask.child {
                ...
                subtask::TaskChild::None => {
                    match subtask.launch() {
                        Ok(_) => {
                            *proc_limit -= 1;
                        ...
                        Err(e) => return TaskResult::Err(format!("ERROR: Failed to launch SubTask process: {e}")),
                    }
                }
            }
        }
...
impl Task<TaskRunning> {
    ...
    pub fn poll(mut self, proc_limit: &mut u32)
        ...
        if anyrunning { return TaskResult::Unchanged(self); }
        if self.itask.subtasks.iter().any(
            |st|
            match &st.child {
    ...
    pub fn check(mut self) -> TaskResult<Task<TaskComplete>, (String,Times)> {
        ...
        match fs::metadata(&successpath) {
            Err(error) => match error.kind() {
                ErrorKind::NotFound => println!( // this is what we want, someone cleaned up the output link
                    "Noted completion of client for taskfile {successpath:?}"
                ),
                _ => return TaskResult::Err(format!(
        ...
        TaskResult::Transformed((String::from(&self.itask.taskdef.name),self.itask.times.clone()))
    }
}
...
fn format_line(format: &str, values: &HashMap<String,String>) -> Result<String,String> {
    ...
    for key in BRACED_NAME_REGEX.captures_iter(format) {
        if ! values.contains_key(&key["name"]) {
            return Err(format!("no value associated with {} could be found", &key["name"]));
        }
    }
    Ok(
        String::from(
            BRACED_NAME_REGEX.replace_all(
                format,
                |captures: &regex::Captures| values.get(&captures["name"]).unwrap()
            )
        )
    )
}
